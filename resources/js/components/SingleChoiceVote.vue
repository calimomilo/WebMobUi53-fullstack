...
const props = defineProps({
  ...
  const selectedOption = ref(props.poll.options.find(o => votedIds.value.includes(o.id)));
...
<template>
    <h2 class="text-lg mb-3">{{ props.poll.question }}</h2>
    <div class="flex flex-col gap-2">
        <div v-for="option in props.poll.options" class="py-2 px-3 border-2 rounded-md"
        :class="{ 
